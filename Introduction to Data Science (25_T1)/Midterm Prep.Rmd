...
# Color of circumference is red
# c("red","blue","orange") defines colors filling in circles
...
Create extended version of circle function that has default border and fill colors:
```{r}
circle_ext <- function(x,y,
                       border_colour = 'red', # default border color
                       colour = c('red', 'blue', 'orange')){ # default fill colors
  ...
  draw.circle(x,y,c(1,0.66,0.33),
              border=border_colour, # border color passed by the user
              col=colour, # fill colors passed by the user
              lty=1,
              lwd=1)
}
...
murders %>% ggplot(aes(x = population/10^6, y = total, label = abb)) +
  geom_point(aes(
    # assign color based on geographical region
    col = region),
    size = 3
    ) +
  geom_text(nudge_x = 0.05) +
...
scale_colour_manual() sets manual values for the color categorical factor in terms of a vector of colors
...
savings_data %>%
  ...
  scale_color_manual(values = c("Low Savings" = "blue",
...
uk_gdp_and_weeklyWage %>%
  ...
  geom_point(aes(y=GDP_per_cap/scale_factor), size = 2, color = "red", alpha = 0.7, shape = 17) +
  geom_line(aes(y=GDP_per_cap/scale_factor), color = "red", linetype = "dashed") +
...
Instead of labs(colour="Title"), to change the legend, we can do scale_color_discrete(name="Title") or theme(legend.title=element_text("Title"))
```{r}
r <- murders %>%
...
To make any url links have the distinct blue colour, add “urlcolor: blue” to the YAML header.
...
fill defines the fill color of the plots
...
scale_color_manual() sets the mappings from data to color
...
inner_join_data1 %>%
  ...
  scale_color_manual(values = c("England" = "darkorange",
                                "Wales" = "darkgreen")) +
